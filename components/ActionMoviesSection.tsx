...
export default function ActionMoviesSection() {
  ...
  useEffect(() => {
    async function fetchMovies() {
      try {
        // Fetch action movies (genre_id 28)
        const response = await fetch(
          `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&with_genres=28&sort_by=popularity.desc&page=1`
        )
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[#c9a84c] text-xs uppercase tracking-widest font-semibold">Action Packed</span>
              </div>
              <h2 className="font-display text-2xl font-bold text-[#e8e8e8] mt-0.5">Action & Thrillers</h2>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
