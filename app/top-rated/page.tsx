...
export default function TopRatedPage() {
  ...
  useEffect(() => {
    async function fetchTopRated() {
      try {
        // Load first 3 pages (60 movies)
        const [page1, page2, page3] = await Promise.all([
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=1`).then(r => r.json()),
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=2`).then(r => r.json()),
          fetch(`https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=3`).then(r => r.json()),
        ])
  ...
  async function loadMore() {
    ...
    try {
      ...
      const response = await fetch(
        `https://api.themoviedb.org/3/movie/top_rated?api_key=${process.env.NEXT_PUBLIC_TMDB_API_KEY}&page=${nextPage}`
      )
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-[#9ca3af] mt-1">The highest rated movies of all time</p>
          </div>
        </div>
